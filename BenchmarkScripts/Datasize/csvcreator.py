...
headers = ["Benchmark Method", "Amount of Nodes", "Size per Node", "Data Size (Bytes)",
           "Ops/sec", "Hits/sec", "Misses/sec", "Avg. Latency", "p50 Latency", "p99 Latency", 
           "p99.9 Latency", "KB/sec"]

def parse_filename(file_name):
    match = re.match(r"(\w+)_(\d+)x(\d+)gb_(\d+)(bsize|datasize)", file_name)
    if match:
...
for file_name in os.listdir(BENCHMARK_DIR):
    if file_name.endswith(".txt"):
        ...
        if None not in benchmark_info:
            ...
            print(f"Skipping file due to pattern mismatch: {file_name}")
